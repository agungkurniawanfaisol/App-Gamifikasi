    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border/60 bg-background/95 backdrop-blur-md supports-[backdrop-filter]:bg-background/80 md:sticky">
      <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4 sm:px-6">
        {isLanding ? (
          ...
          <Link
            href="/"
            className="inline-flex min-w-0 shrink-0 items-center gap-1.5 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
          >
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <Button
            ...
            size="sm"
            className={cn(page === "login" && "pointer-events-none")}
          >
          ...
          >
            <Link href="/register" aria-current={page === "register" ? "page" : undefined}>
              {labels.auth.signUp}
            </Link>
          </Button>
        </div>
